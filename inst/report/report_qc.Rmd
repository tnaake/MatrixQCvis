...
if (!is.null(params$sample_mosaic_f1)) {
    ...
    called <- FALSE
    sample_mosaic_f1 <- colnames(colData(params$se_r))[1]
}
if (!is.null(params$sample_mosaic_f2)) {
    sample_mosaic_f2 <- params$sample_mosaic_f2
} else {
    sample_mosaic_f2 <- colnames(colData(params$se_r))[1]
}
...
hist_feature(assay(se_r), measured = TRUE, binwidth = binwidth)  
```
...
if (!is.null(params$mev_binwidthC)) {
    ...
    called <- FALSE
    binwidth <- 1
    mev_hist_category <- colnames(colData(se_r))[1]
}
```
...
if (!is.null(params$mev_upset_category)) {
    ...
    called <- FALSE
    mev_upset_category <- colnames(colData(se_r))[1]
}
...
hist_feature(assay(se_r), measured = FALSE, binwidth = binwidth)
```
...
if (!is.null(params$miv_binwidthC)) {
    ...
    called <- FALSE
    binwidth <- 1
    miv_hist_category <- colnames(colData(se_r))[1]
}
```
...
if (!is.null(params$miv_upset_category)) {
    ...
    called <- FALSE
    miv_upset_category <- colnames(colData(se_r))[1]
}
```
...
create_boxplot(assay(se_r), title = "raw",
    log2 = int_log, violin = int_violin)
create_boxplot(assay(se_n), title = "normalized",
    log2 = int_log, violin = int_violin)
create_boxplot(assay(se_t), title = "transformed",
    log2 = FALSE, violin = int_violin)
create_boxplot(assay(se_b), title = "batch corrected",
    log2 = FALSE, violin = int_violin)
create_boxplot(assay(se_i), title = "imputed",
    log2 = FALSE, violin = int_violin)
```
...
if (!is.null(params$int_drift_data)) {
    ...
    int_drift_category <- colnames(colData(se_r))[1]
    int_drift_orderCategory <- colnames(colData(se_r))[1]
    int_drift_level <- "all"
    int_drift_method <- "loess"
}
```
...
if (!is.null(params$int_ecdf_label)) {
    ...
    called <- FALSE
    int_dist_label <- colnames(colData(se_r))[1]
    int_dist_method <- "euclidean"
}
```
...
if (missingValue) {
    l_assays <- list(raw = a, normalized = a_n, 
...
ordinationPlot(tbl = tbl, se = se_r, highlight = dr_pca_highlight,
    x_coord = dr_pca_x, y_coord = dr_pca_y)
```
...
ordinationPlot(tbl = tbl, se = se_r, highlight = dr_pcoa_highlight,
    x_coord = dr_pcoa_x, y_coord = dr_pcoa_y)
```
...
ordinationPlot(tbl = tbl, se = se_r, highlight = dr_nmds_highlight,
    x_coord = dr_nmds_x, y_coord = dr_nmds_y)
```
...
ordinationPlot(tbl = tbl, se = se_r, highlight = dr_tsne_highlight,
    x_coord = dr_tsne_x, y_coord = dr_tsne_y)
```
...
ordinationPlot(tbl = tbl, se = se_r, highlight = dr_umap_highlight,
    x_coord = dr_umap_x, y_coord = dr_umap_y)
```
...
if (valid_m & valid_c) {
    mM <- model.matrix(m_formula, data = colData(se_r))
    cM <- makeContrasts(contrasts = c_formula, levels = mM)

    if (params$de_method == "limma") {
        tT <- topTable(params$de_fit_ttest, number = Inf, adjust.method = "fdr", p.value = 0.05)
        tT <- cbind(name = rownames(tT), tT)
        volcanoPlot(tT, type = "ttest")
    }
    if (params$de_method == "proDA") {
        tT <- test_diff(fit = params$de_fit_proDA, contrast = params$de_c_formula,
            sort_by = "adj_pval")
