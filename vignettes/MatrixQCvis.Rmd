...
author: 
 - name: Thomas Naake and Wolfgang Huber
   mail: thomas.naake@embl.de, wolfgang.huber@embl.de
   affiliation: European Molecular Biology Laboratory, Meyerhofstrasse 1, 69117 Heidelberg
package: MatrixQCvis
abstract: >
    MatrixQCvis comprises functionality to assess the quality of omics data.
...
`MatrixQCvis` includes functionality for interactive, `shiny`-based visualization 
of diagnostic plots that allow for the assessment of data quality. 
Thus, it allows to identify low-quality samples and exclude them. `MatrixQCvis` builds
upon the popular Bioconductor class of `SummarizedExperiment` and enables for 
facile integration into existing workflows. 
...
allows for imputation
of missing values and differential expression analysis using the `proDA` 
package [@Ahlmann2019]. Besides this, it is extensible to other type of data
(e.g. transcriptomics count data) that can be represented as a 
...
Visualizations include bar- and violin plots of the
(count/intensity) values, mean-sd plots, MA plots, empirical cumulative distribution 
function (ECDF) plots, plots to visualize the
distance between samples, multiple types of dimension reduction plots, and more. 
...
[GitLab](https://git.embl.de/naake/MatrixQCvis) or
...
transcripts, peptides, proteins, or metabolites) and the column represent the 
sample. The 
...
- for a `SummarizedExperiment` containing missing values
...
- for a `SummarizedExperiment` containing no missing values (i.e.
...
## Sidebar
The sidebar allows to exclude samples from the analysis (e.g. in the case 
of identified outliers). Samples are excluded by entering the sample name 
in the input field and pressing `Enter` on the keyboard or by clicking 
on the name of the samples in the drop-down menu. 
...
In this case, the samples show more or less the same number of measured features
and there is no indication to remove any sample based on this measure. 
...
The type of visualization (boplot or violin plot) can be specified by 
...
For the raw values, the mean-sd plot will look like
```{r meansdplot, eval=TRUE, echo=FALSE, warning=FALSE}
vsn::meanSdPlot(assay(se))
```
...
The values are defined as follows
 ...
 - $M =  I_i−I_j$ , where $I_i$ and $I_j$  are transformed values. 
...
`proDA` [@Ahlmann2019] was devloped for the differential abundance analysis
