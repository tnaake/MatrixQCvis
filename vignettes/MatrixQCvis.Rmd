...
library("knitr")
opts_chunk$set(stop_on_error = 1L, out.width = "80%")
...
## here we will restrain the analysis on 100 samples and remove the features
...
```{r hist_sample, eval=TRUE, echo=FALSE, crop=NULL}
...
```{r mosaic, eval=TRUE, echo=FALSE, crop=NULL}
se_tmp <- se
...
```{r boxplot, eval=TRUE, echo=FALSE, warning=FALSE, crop=NULL}
...
```{r drift, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, crop=NULL}
...
```{r cv, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, crop=NULL}
a <- assay(se)
...
```{r meansdplot, eval=TRUE, echo=FALSE, warning=FALSE, crop=NULL}
...
```{r maplot, eval=TRUE, echo = FALSE, show=FALSE, crop=NULL}
...
```{r ecdf, eval=TRUE, echo=FALSE, crop=NULL}
...
```{r distSampe, eval=TRUE, echo=FALSE, crop=NULL}
a <- assay(se)
...
```{r featurePlot, eval=TRUE, echo=FALSE, crop=NULL}
...
```{r cvFeaturePlot, eval=TRUE, echo=FALSE, crop=NULL}
...
The output will look like the following (the output will only show the first 
10 entries)
...
The output will be (only the first 50 rows will be shown here):
...
rmarkdown::paged_table(tT[seq_len(50), ])
```
...
```{r volcano, eval=TRUE, echo=FALSE, crop=NULL}
...
tbl_mean <- left_join(tbl_info, tbl_mean,
    by = c("protein_id" = "protein_id")) 

## create assay object
cols_a_start <- which(colnames(tbl_mean) == "22RV1_PROSTATE")
...
```{r samplesMeasuredMissing, eval=TRUE, echo=FALSE, crop=NULL}
...
```{r histFeature_measured, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, crop=NULL}
...
```{r histFeature_missing, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, crop=NULL}
...
```{r histFeatureCategory, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, crop=NULL}
...
```{r upsetmeasured, eval=TRUE, echo=FALSE, crop=NULL}
...
```{r upsetmissing, eval=TRUE, echo=FALSE, crop=NULL}
