...
abstract: >
   ...
   enables the detection of low-quality samples, drifts, outliers and batch 
   effects in data sets and their subsequent removal. Visualizations include 
   amongst others boxplots and violin plots of the (count/intensity) 
   ...
   differential expression analysis based on the `limma` (moderated t-tests) 
   and `proDA` (Wald tests) packages. `MatrixQCvis` 
   builds upon the Bioconductor `SummarizedExperiment` S4 class and 
...
We present here the `MatrixQCvis` package that allows for shiny-based 
interactive visualization and assessment of data quality. The application 
enables the detection of low-quality samples, drifts, outliers and batch effects 
in data sets and their subsequent removal. Visualizations include amongst others 
boxplots and violin plots of the (count/intensity) values, mean-sd plots, 
