...
a <- assay(se)
se_t <- se
a_n <- a %>% normalizeAssay(., "sum")
a_t <- a_n %>% transformAssay(., "vsn")
assay(se_t) <- a_t
a_b <- se_t %>% batchCorrectionAssay(., method = "none")
a_i <- a_b %>% imputeAssay(., "MinDet")
...
user to track the influence of 
