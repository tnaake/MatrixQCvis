...
test_that("histSampleServer", {
    shiny::testServer(app = histSampleServer, expr = {
        input <- new.env()
        output <- new.env()
        session <- new.env()
        se <- new.env()

        out <- histSampleServer("test_module", se = se)
...
test_that("mosaicSampleServer", {
    shiny::testServer(app = mosaicSampleServer, expr = {
        input <- new.env()
        output <- new.env()
        session <- new.env()
        se <- new.env()

        out <- mosaicSampleServer("test_module", se = se)
        expect_is(out, "shiny.render.function")
    })
})
